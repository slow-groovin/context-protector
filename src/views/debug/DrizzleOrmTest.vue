<template>
  <div class="min-h-screen bg-gray-100 py-8">
    <div class="max-w-4xl mx-auto px-4">
      <h1 class="text-3xl font-bold text-gray-800 mb-8">Drizzle ORM Test</h1>
      ...
      <div class="space-y-6">
        ...
        <!-- Drizzle ORM Tests -->
        <div class="bg-white rounded-lg shadow p-6">
          <h2 class="text-xl font-semibold mb-4">Drizzle ORM Tests (Schema-Driven)</h2>
          <div class="space-y-4">
...
const initDB = async () => {
  ...
  try {
    await initDatabase()
    addMessage('success', 'Database initialized successfully with Drizzle ORM')
  } catch (error: any) {
...
const runDBMigrations = async () => {
  ...
  try {
    await runMigrations()
    addMessage('success', 'Database migrations completed successfully with Drizzle schema')
  } catch (error: any) {
...
const testCRUD = async () => {
  ...
  try {
    ...
    const newContext = await db.insert(contexts).values({
      ...
      content: 'This is a test context content for demonstrating Drizzle ORM capabilities',
      createdAt: new Date().toISOString()
    }).returning()

    addMessage('success', `Created context with Drizzle: ${newContext[0]?.name}`)

    // Query with filters
    const allUsers = await db.select().from(users).where(eq(users.name, 'Drizzle User'))
    ...
    addMessage('success', 'CRUD operations completed successfully with Drizzle ORM')
  } catch (error: any) {
...
const testRelations = async () => {
  ...
  try {
    ...
    addMessage('success', `Queried ${usersWithTasks.length} users with their tasks using relations`)
    ...
    addMessage('success', `Queried ${tasksWithUsers.length} tasks with their users using relations`)
    ...
    addMessage('success', `Found ${completedTasks.length} completed tasks with user info`)
    ...
    addMessage('success', 'Relations and joins tested successfully with Drizzle ORM')
  } catch (error: any) {
...
onMounted(() => {
  ...
  addMessage('info', 'This page demonstrates full Drizzle ORM capabilities including relations and schema-driven operations')
})
</script>